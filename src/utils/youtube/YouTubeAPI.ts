...
export class YouTubeAPI {
  ...
  public static getPlayListVideos(
    playListId: string,
    callback: (videos: string[]) => void
  ) {
  ...
  public static getVideoThumbnail(
    videoId: string,
    callback: (image: string) => void,
    size: string
  ) {
    fetch(
      `https://youtube.googleapis.com/youtube/v3/videos?part=snippet&id=${videoId}&key=${this.API_KEY}`
    )
      .then((response) => response.json())
      .then((data) => {
  ...
  public static getVideoSnippetInfo(
    videoId: string,
    snippetName: string,
