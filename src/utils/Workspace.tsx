...
export class Workspace {
    ...
    public static deployProject() {
        Workspace.preview();
        UIManager.alert("<h1>Projekt Veröffentlicht!</h1><h4><pre><code><a target='_blank' href='preview.html?code=" + Options.currentLiveJS + "'>Öffnen</a></code></pre></h4>")
    }
    ...
    public static deployAccount() {
        ...
        if (UserUtil.getSavedUser().username && UserUtil.getSavedUser().password) {
            ...
            User.storeData_u(
                ...
                JSON.parse(localStorage.getItem("incode-editor.projects." + Registry.getRegister(0x10AD)) as string),
                "project-" + Registry.getRegister(0x10AD)
            ).then(r => {
                if (r) {
                    UIManager.alert("<h1>Erfolgreich</h1><h4>Das Projekt wurde in deinem Account gespeichert.</h4>")
                }
            })
        } else {
            UIManager.ask("<h1>Achtung!</h1><h4>Du bist nicht angemeldet! Willst du dich anmelden?</h4>", () => {
                Workspace.save(false);
