...
export class SelectLoginOption extends React.Component {
          ...
          <div className={"menu-choose-editors-root"}>
            <div className={"menu-choose-editor"} onClick={this.login}>
              <img
                ...
                height={128}
              />
              <p className={"menu-editor-description"}>Anmelden</p>
            </div>
            <div className={"menu-choose-editor"} onClick={this.register}>
              <img
                ...
                height={128}
              />
              <p className={"menu-editor-description"}>Registrieren</p>
            </div>
          </div>
        </div>
      </>
  ...
  login() {
    function x() {
      UIManager.prompt(
        "<h1>Anmelden</h1><h4>Bitte gib deinen Nutzernamen ein</h4>",
        (username) => {
          UIManager.prompt(
            "<h1>Anmelden</h1><h4>Bitte gib dein Passwort ein</h4>",
            async (password) => {
              const status = await UserUtil.loginAndSave(username, password);
              if (status) {
                UIManager.alert(
                  "<h1>Erfolgreich!</h1><h4>Du bist nun als " +
                    username +
                    " angemeldet.</h4>",
                  () => {
                ...
                UIManager.alert(
                  "<h1>Fehler</h1><h4>Das Passwort oder der Nutzername ist falsch!</h4>",
                  x
                );
              }
            }
  ...
  register() {
    function x() {
      UIManager.prompt(
        "<h1>Registrieren</h1><h4>Bitte gib deinen Nutzernamen ein</h4>",
        (username) => {
          p();

          function p() {
            UIManager.prompt(
              "<h1>Registrieren</h1><h4>Bitte gib dein Passwort ein</h4>",
              async (_password) => {
                UIManager.prompt(
                  "<h1>Registrieren</h1><h4>Bitte gib dein Passwort erneut ein</h4>",
                  async (password) => {
                    if (password === _password) {
                      await c(username, password);
                    } else {
                      UIManager.alert(
                        "<h1>Fehler</h1><h4>Die Passwörter stimmen nich überein!</h4>",
                        () => {
                          p();
                        }
                      );
          ...
          async function c(username: string, password: string) {
            ...
            if (createStatus) {
              ...
              UIManager.alert(
                "<h1>Fehler</h1><h4>Der Nutzername ist vergeben!</h4>",
                () => {
                  x();
                }
              );
