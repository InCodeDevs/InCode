        ...
        if (Firebase.getInstance().isLoggedIn()) {
            UIManager.ask("<h1>Abmelden?</h1><h4>Willst du dich wirklich abmelden?</h4>", () => {
                Firebase.getInstance().logout(() => {
                    (document.getElementById('userIndicatorPicture') as HTMLImageElement).src = "https://us.123rf.com/450wm/tuktukdesign/tuktukdesign1608/tuktukdesign160800043/61010830-user-icon-mann-profil-gesch%C3%A4ftsmann-avatar-person-glyph-vektor-illustration.jpg";
                    (document.getElementById('userIndicatorAs') as HTMLDivElement).innerText = "Anmelden";
                });
            })
        } else {
            Firebase.getInstance().loginGoogle((user) => {
                if(user != null){
                    (document.getElementById('userIndicatorPicture') as HTMLImageElement).src = user.user.photoURL as string;
                    (document.getElementById('userIndicatorAs') as HTMLDivElement).innerText = user.user.displayName as string;
                }
            })
        }
    }
