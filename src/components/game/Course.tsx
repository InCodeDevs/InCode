...
export class Course extends React.Component<CourseProps, CourseState> {
        ...
        return (
            <div className={"tutorials"}>
                ...
                <div className={"tutorials-content"}>
                    ...
                    <div id={"tutorials-status"}><h4 style={{color: 'red'}}>Du hast noch kein Tutorial geöffnet. Klicke Links auf ein Tutorial um es dir anzuschauen!</h4></div>
                    <div id={"yt-video"}/><br />
                </div>
            </div>
        );
    ...
    componentDidMount() {
        ...
        if(localStorage.getItem("incode-editor.tutorials.watched") === null)
            localStorage.setItem("incode-editor.tutorials.watched", JSON.stringify({videos:[]}))
        ...
        YouTubeAPI.getPlayListVideos(playlistId, (videos) => {
            videos.forEach(id => {
                ...
                YouTubeAPI.getVideoThumbnail(id, (image) => {
                    img.src = image;
                    img.width = 120;
                    img.height = 90;
                }, "high")
                ...
                div.addEventListener('click', () => {
                    (document.getElementById('tutorials-status') as HTMLElement).style.display = 'none';
                    (document.getElementById('yt-video') as HTMLElement).style.display = 'block';
                    (document.getElementsByClassName('tutorials-section-title')[0] as HTMLElement).innerText = title.innerText;
                    YouTubePlayerAPI.play(id, () => {

                        (document.getElementById('yt-video') as HTMLElement).style.display = 'none';

                        (document.getElementById('tutorials-status') as HTMLElement).style.display = 'block';
                        (document.getElementById('tutorials-status') as HTMLElement).innerHTML =
                ...
                (document.getElementsByClassName('tutorials-sidebar-entries')[0] as HTMLUListElement).appendChild(div)
            })
        })
    }
}